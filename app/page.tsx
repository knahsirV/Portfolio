...
import { otherSections, heroContainer, heroItem } from "./ConstData/animations";
      ...
      <main className='mx-auto min-h-screen max-w-6xl p-6 lowercase md:w-3/4 md:p-10'>
        <motion.div initial='hidden' animate='visible' variants={heroContainer}>
          <motion.section
            variants={heroItem}
            id='hero'
            className='mb-16 grid place-content-center md:h-[45vh]'
          >
            <div className='mb-6 flex flex-wrap items-center justify-center gap-4'>
              {quickActions.map((action) => (
                <motion.div variants={heroItem} key={action.name}>
                  <Link
                    href={action.link}
                    target='_blank'
